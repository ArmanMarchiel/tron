...
export function renderRobotState(el, t) {
  ...
  <table class="kv">
    ...
    <tr><td>Task step</td><td>${t.task?.step ? `'${t.task.step}' (${t.task.action}) · ${t.task.status}` : "—"}</td></tr>
    ...
    <tr><td>Safety</td><td>${p.protective_stop ? '<span class="bad">PROTECTIVE STOP</span>' : "normal"} · scanner ${t.sensors.area_scanner?.intrusion == null ? "—" : (t.sensors.area_scanner.intrusion ? '<span class="bad">intrusion</span>' : "clear")}${p.human_in_scanner_field ? ' <span class="dim">(person physically in field)</span>' : ""}</td></tr>
